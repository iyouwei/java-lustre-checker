...
struct_expr: '{' label_expr (',' label_expr)* '}';  // 修正：逗号分隔
...
pattern: ID | CHAR | '-'? INTEGER | 'true' | 'false' | '_';
...
list: (simple_expr (',' simple_expr)*)?;  // 修正：逗号分隔
...
// Atoms
atom: 'true' | 'false' | CHAR | INTEGER | UINT | FLOAT | REAL | USHORT | SHORT;
